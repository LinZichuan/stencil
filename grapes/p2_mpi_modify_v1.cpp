...
#define IDXx(i,j,k  ) (i)*(ny+2)*(nz+2)+(j)*(nz+2)+(k)
...
void gcr(double *x, double *A, double *b, int nx, int ny, int nz, int px, int py, int NX, int NY, int PX, int PY) {
    //sync x
    int size = nx*ny*nz;
...
int main(int argc,char**argv) {
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    gcr(x_, A_, b_, nx_, ny_, nz_, px, py, NX, NY, PX, PY);
    MPI_Barrier(MPI_COMM_WORLD);
