...
void gcr(double *x, double *A, double *b, int nx, int ny, int nz, int px, int py, int NX, int NY, int NZ, int PX, int PY) {
    ...
    double *R_ = new double[sizex];
    double *p0 = new double[sizex];
    ...
    for (int ii = 0; ii < nx; ++ii) {
        for (int jj = 0; jj < ny; ++jj) {
            for (int zz = 0; zz < nz; ++zz) {
                int r_idx = (ii+1)*(ny+2)*(nz+2) + (jj+1)*(nz+2) + zz+1;
                int ridx = ii*ny*nz + jj*nz + zz;
                //R_[r_idx] = R[ridx];
                p0[r_idx] = R_[r_idx];
            }
        }
    ...
    for (int i = 1; i < 100; ++i) {
        ...
        for (int ii = 0; ii < nx; ++ii) {
            for (int jj = 0; jj < ny; ++jj) {
                for (int zz = 0; zz < nz; ++zz) {
                    int idx = (ii+1)*(ny+2)*(nz+2) + (jj+1)*(nz+2) + zz+1;
                    x[idx] = x[idx] + alpha*vecp[i-1][idx]; //p + 1
                }
            }
        }
            ...
            for (int i1 = 0; i1 < nx; ++i1) {
                for (int j1 = 0; j1 < ny; ++j1) {
                    for (int k1 = 0; k1 < nz; ++k1) {
                        int idx = (i1+1)*(ny+2)*(nz+2) + (j1+1)*(nz+2) + k1+1;
                        p[idx] += beta[j-jbegin] * vecp[j][idx];
                    }
                }
            }
        }
        ...
        //    }
        //}
        //double *t1 = sigma(beta, vecp, jbegin, i, size);
...
#define local_idxx(i1,j1,k1) (((i1)*(ny_+2)*(nz_+2))+((j1)*(nz_+2))+(k1))
    for (int i = 0; i < nx_; ++i) {
        for (int j = 0; j < ny_; ++j) {
            for (int k = 0; k < nz_; ++k) {
                ...
                x_[local_idxx(i+1,j+1,k+1)] = x[global_idx];
            }
        }
    }
