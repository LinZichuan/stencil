...
int main(int argc, char **argv){
	...
	int partsize = sizeof(REAL)*NX_*NY*NZ;
    ...
    for (int part = 0; part < num; part++)
        for (int k = 0; k < NZ; k++)
            for (int j = 0; j < NY; j++)
                for (int i = 0; i < NX_; i++) {
                    host_A[part][k*NY*NX_+j*NX_+i] = 1.0;	
                    host_B[part][k*NY*NX_+j*NX_+i] = 1.0;
                }
    ...
    for (int i = 0; i < num; ++i) {
        ...
        dim3 blockPerGrid((NX_+BX-1)/BX, (NY+BY-1)/BY, GZ); //512/128,512/1,1 = 4,512,1

        ///////////////////////////////////////////////////////////////
        //baseline
        for (int t = 0; t < T; t++){
            baseline<<<blockPerGrid, threadPerBlock>>>(dev_A, dev_B, NX_, NY, NZ);		
            REAL* tmp = dev_A;
            dev_A = dev_B;
            dev_B = tmp;
        }
    ...
    int begin, end;
    int smallsize = NX/num * NY * NZ;
    int i=0, z=0, y=0, x=0;
    for (i = 0; i < num; ++i) {
        begin = 1;
        end = NX_-1;
        if (i == 0) { begin=0; end=NX_-2; } 
        else if (i == num-1) { begin=2; end=NX_; }
        int index = i*smallsize;
        for (z = 0; z < NZ; ++z)
            for (y = 0; y < NY; ++y)
                for (x = begin; x < end; ++x) {
                    result_A[index] = host_A[i][NY*NX_*z + y*NX_ + x];
                    assert(abs(host_A[i][NY*NX_*z + y*NX_ + x] - 1.0) < 1e-5);
                    //if (i == 2)
    ...
    for (int i = 0; i < tmpp; ++i) {
        //assert(abs(result_A[i] - 1.0) < 1e-5);
    }
